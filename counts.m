...
tp_max = find(t1>=3.33,1);

%Time pulse finishes in clock time
%thp = find(t1>=3.47,1);
thp =ceil((tp+t1gate)/dt)+1;
...
for i=tlp:thp
    ...
    for j=1:tn
        t1i=find(t>=(j-1)*pt+t1(i)+tb,1)-1;
        ti=t(t1i);
        t1f=find(t>=(j-1)*pt+t1(i)+te,1)-1;
        t2=t(t1f);
...
for i=tlp:thp
    for j=1:tn-1
        t1i=find(t>=-(j)*pt+t1(i)+tb,1)-1;
        ti=t(t1i);
        t1f=find(t>=-(j)*pt+t1(i)+te,1)-1;
        t2=t(t1f);
        dtau=t2-ti;
        g3c(i-tlp+1, t1i:t1f)=g3(i,t1i:t1f);
    end
